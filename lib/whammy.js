...
module.exports = (function(){
	...
	function generateEBML(json, outputAsArray){
		...
		if(outputAsArray){
			...
			return new window.Blob(ebml, {type: "video/webm"});
		}
	}
	...
	WhammyVideo.prototype.compile = function(outputAsArray){
		return new toWebM(this.frames.map(function(frame){
			var webp = parseWebP(parseRIFF(window.atob(frame.image.slice(23))));
	...
	return {
		...
		fromImageArray: function(images, fps, outputAsArray){
			return toWebM(images.map(function(image){
				var webp = parseWebP(parseRIFF(window.atob(image.slice(23))))
